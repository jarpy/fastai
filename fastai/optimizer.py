...
def SGD(params, lr, mom=0.0, wd=0.0, decouple_wd=True):
...
def RMSProp(params, lr, sqr_mom=0.99, mom=0.0, wd=0.0, decouple_wd=True):
...
def Larc(
    params, lr, mom=0.9, clip=True, trust_coeff=0.02, eps=1e-8, wd=0.0, decouple_wd=True
):
    "A `Optimizer` for Adam with `lr`, `mom`, `sqr_mom`, `eps` and `params`"
    cbs = [weight_decay] if decouple_wd else [l2_reg]
    if mom != 0.0:
...
def Lamb(params, lr, mom=0.9, sqr_mom=0.99, eps=1e-5, wd=0.0, decouple_wd=True):
    "A `Optimizer` for Adam with `lr`, `mom`, `sqr_mom`, `eps` and `params`"
    cbs = [weight_decay] if decouple_wd else [l2_reg]
    cbs += [
        partial(average_grad, dampening=True),
...
class OptimWrapper(_BaseOptimizer, GetAttr):
    ...
    def __init__(
        self,
        params: list
        | dict = None,  # Model parameters to pass to `opt`. If using an already built `opt`
        opt: callable
